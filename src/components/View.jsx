...
import { getAllAPIVideosAPI, saveVideoAPI, updateCategoryAPI } from '../services/allAPI'
...
const View = ({addResponseFromHome,deleteResponseFromCategory,setDeleteResponseFromView}) => {
  ...
  const getAllVideos = async ()=>{
    try {
      const result = await getAllAPIVideosAPI()
      console.log(result);
  ...
  const categoryVideoDropOverView= async(e)=>{
    ...
    // update the category by deleting video from category using api
